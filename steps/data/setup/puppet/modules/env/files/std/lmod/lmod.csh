...
set MY_NAME="/etc/profile.d/lmod.csh"
...
if ( ! $?MODULEPATH_ROOT ) then
    ...
    setenv MODULEPATH_ROOT      "/usr/modulefiles"
    ...
    set APPEND_PATH=`sed -n 's/[   #].*$//; /./H; $ { x; s/^\n//; s/\n/ /g; p; }' /etc/lmod/modulespath`
    foreach i ( `echo $APPEND_PATH` )
        setenv MODULEPATH           `/usr/share/lmod/lmod/libexec/addto --append MODULEPATH $i`
    end
